...
# ╔═╡ 37c08670-9e5f-11ee-29dd-7b69c4a26b99
begin
	using PlutoUI
	using LinearAlgebra
	using ForwardDiff
...
# ╔═╡ 1565a926-bec5-4b97-8ecb-f3322d090553
#=╠═╡
begin
function example_joni(x,rng)
	return -2*x.^3+4x.^2-x+1/4*randn(rng,size(x))
end
	x_values=collect(range(-.5,2,201))
	f_values=example_joni(x_values,rng)
end;
  ╠═╡ =#

# ╔═╡ 63e82dd7-2956-4db1-92aa-454d4244406e
#=╠═╡
with_theme(theme_web()) do
    fig = Figure()
    ax = CairoMakie.Axis(fig[1,1]; xlabel="x", ylabel="y")

    l = lines!(ax, x_values[:], x_values -> evalpoly(x_values, (0, -1, 4, -2)); linewidth=3)
    s = scatter!(ax, x_values[:], f_values[:]; markersize=8, color=:orange,
        strokecolor=:black, strokewidth=1)

    axislegend(ax, [l, s], ["True Cubic Function", "Data Points"])

    return fig
end
  ╠═╡ =#

# ╔═╡ 6f49dea4-ac6f-416c-922d-76109b179484
begin
	model = Chain(Dense(1 => 4, relu), Dense(4 => 8, relu),Dense(8 => 4, relu),Dense(4 => 1))
	opt = Adam(0.03f0)
end

# ╔═╡ 24506f1c-17f2-4662-a7ab-b76aa070dd68
Lux.parameterlength(model)

# ╔═╡ 63131b96-1cee-4bb9-a28e-eecc8b2071f6
function grad_log_p(theta,data)
	result=0
	size_batches=10
	num_batches=ceil(size(data,1)/size_batches)
	for i =1:num_batches-1
		batch=data[size_batches*(i-1)+1:size_batches*i]
		result+=liklelihood(theta,batch)
	end
	batch=data[size_batches*(num_batches-1)+1:size(data,1)]
	result+=liklelihood(theta,batch)
	result=(size(data,1)/num_batches)*result+log(prior(theta))
	return result
end

# ╔═╡ 9934638c-951f-446f-aafa-70fb109d1427
function loss_function(model, ps, state, data)
    y_pred, state = Lux.apply(model, data[1], ps, state)
    mse_loss = mean(abs2, y_pred .- data[2])
    return mse_loss, state, ()
end

# ╔═╡ e5e7901f-9f76-457b-912f-0459366fb283
#=╠═╡
begin
	...
	tstate = main(tstate, vjp_rule, (x_values', f_values'), 250)
	y_pred = dev_cpu(Lux.apply(tstate.model, dev_gpu(x_values'), tstate.parameters, tstate.states)[1])
end;
  ╠═╡ =#

# ╔═╡ 63612b2d-4d4f-4254-9000-ea2de73185eb
#=╠═╡
with_theme(theme_web()) do
    fig = Figure()
    ax = CairoMakie.Axis(fig[1, 1]; xlabel="x", ylabel="y")
    ...
    l = lines!(ax, x_values[:], x_values -> evalpoly(x_values, (0, -1, 4, -2)); linewidth=3)
    s1 = scatter!(ax, x_values[:], f_values[:]; markersize=8, color=:orange,
        strokecolor=:black, strokewidth=1)
    s2 = scatter!(ax, x_values[:], y_pred[:]; markersize=8, color=:green,
        strokecolor=:black, strokewidth=1)

    axislegend(ax, [l, s1, s2], ["True Cubic Function", "Actual Data", "Predictions"])

    return fig
end
  ╠═╡ =#

# ╔═╡ 4eccbac5-0f16-4531-8f78-79532a632f80
#=╠═╡
begin
	M=100
...
# ╔═╡ c625badf-6f98-45fa-b2e7-f20e9cdcadc5
#=╠═╡
begin
...
# ╔═╡ 974ce352-e46a-4f94-855b-dbd0917c6e13
#=╠═╡
begin
	rng =  MersenneTwister()
...
# ╔═╡ a1056c8c-13e4-4f7a-b86e-8c845e123a43
# ╠═╡ disabled = true
#=╠═╡
begin
...
# ╔═╡ d48117b8-e095-4dc3-974a-eefabb51fe57
#=╠═╡
function vector_to_parameters(ps_new::AbstractVector, ps::NamedTuple)
    @assert length(ps_new) == Lux.parameterlength(ps)
    i = 1
...
# ╔═╡ 26e74752-c792-4a95-98c0-456ae06e7082
#=╠═╡
begin
...
julia_version = "1.9.3"
manifest_format = "2.0"
project_hash = "62370d6dabaf23ad4256ecf7d79d60b3a20697ca"
...
git-tree-sha1 = "a8e84d2dec289f03ca054be3814de50f2312f0e1"
...
git-tree-sha1 = "64a7219e4bf18bdc45004786d920c0f46d98cf52"
